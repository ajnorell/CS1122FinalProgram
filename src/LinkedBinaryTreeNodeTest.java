...
public class LinkedBinaryTreeNodeTest {
   ...
   public void testPathTo() throws Exception {
      ...
      if (!path1.get(0).equals(parent)) {
         fail(String.format("(%s).pathTo(%s).get(0) returned %s should be %s from path %s", parent, greatGrandChild,
               path1.get(0), parent, path1));
      }
      if (!path1.get(1).equals(rightChild)) {
         fail(String.format("(%s).pathTo(%s).get(1) returned %s should be %s from path %s", parent, greatGrandChild,
               path1.get(1), rightChild, path1));
      }
      if (!path1.get(2).equals(grandChildC)) {
         fail(String.format("(%s).pathTo(%s).get(2) returned %s should be %s from path %s", parent, greatGrandChild,
               path1.get(2), grandChildC, path1));
      }
      if (!path1.get(3).equals(greatGrandChild)) {
         fail(String.format("(%s).pathTo(%s).get(3) returned %s should be %s from path %s", parent, greatGrandChild,
               path1.get(3), greatGrandChild, path1));
      }
      ...
      if (!path2.get(0).equals(leftChild)) {
         fail(String.format("(%s).pathTo(%s).get(0) returned %s should be %s from path %s", leftChild, grandChildA,
               path2.get(0), leftChild, path2));
      }
      if (!path2.get(1).equals(grandChildA)) {
         fail(String.format("(%s).pathTo(%s).get(1) returned %s should be %s from path %s", leftChild, grandChildA,
               path2.get(1), grandChildA, path2));
      }
   ...
   public void testPathFrom() throws Exception {
      ...
      if (!path1.get(3).equals(parent)) {
         fail(String.format("(%s).pathFrom(%s).get(0) returned %s should be %s from path %s", greatGrandChild, parent,
               path1.get(3), parent, path1));
      }
      if (!path1.get(2).equals(rightChild)) {
         fail(String.format("(%s).pathFrom(%s).get(1) returned %s should be %s from path %s", greatGrandChild, parent,
               path1.get(2), rightChild, path1));
      }
      if (!path1.get(1).equals(grandChildC)) {
         fail(String.format("(%s).pathFrom(%s).get(2) returned %s should be %s from path %s", greatGrandChild, parent,
               path1.get(1), grandChildC, path1));
      }
      if (!path1.get(0).equals(greatGrandChild)) {
         fail(String.format("(%s).pathFrom(%s).get(3) returned %s should be %s from path %s", greatGrandChild, parent,
               path1.get(0), greatGrandChild, path1));
      }
      ...
      if (!path2.get(1).equals(leftChild)) {
         fail(String.format("(%s).pathFrom(%s).get(0) returned %s should be %s from path %s", grandChildA, leftChild,
               path2.get(1), leftChild, path2));
      }
      if (!path2.get(0).equals(grandChildA)) {
         fail(String.format("(%s).pathFrom(%s).get(1) returned %s should be %s from path %s", grandChildA, leftChild,
               path2.get(0), grandChildA, path2));
      }
   ...
   public void testRemoveFromParent() throws Exception {
      ...
      if (node != null) {
         fail(String.format("After (%s).removeFromParent( ), (%s).getParent( ) returned %s, should be %s.", leftChild,
               leftChild, node, null));
      }
      node = parent.getLeft();
      if (node != null) {
         fail(String.format("After (%s).removeFromParent( ), (%s).getLeft( ) returned %s, should be %s.", leftChild,
               parent, node, null));
      }
   }
   ...
   public void testTraversePreorder() throws Exception {
      ...
      if (!list1.get(0).equals(parent)) {
         fail(String.format(
               "(%s).traversePreorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(0), 0, parent, list1));
      }
      if (!list1.get(1).equals(leftChild)) {
         fail(String.format(
               "(%s).traversePreorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(1), 1, leftChild, list1));
      }
      if (!list1.get(2).equals(grandChildA)) {
         fail(String.format(
               "(%s).traversePreorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(2), 2, grandChildA, list1));
      }
      if (!list1.get(3).equals(grandChildB)) {
         fail(String.format(
               "(%s).traversePreorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(3), 3, grandChildB, list1));
      }
      if (!list1.get(4).equals(rightChild)) {
         fail(String.format(
               "(%s).traversePreorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(4), 4, rightChild, list1));
      }
      if (!list1.get(5).equals(grandChildC)) {
         fail(String.format(
               "(%s).traversePreorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(5), 5, grandChildC, list1));
      }
      if (!list1.get(6).equals(greatGrandChild)) {
         fail(String.format(
               "(%s).traversePreorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(6), 6, greatGrandChild, list1));
      }
      ...
      if (!list2.get(0).equals(leftChild)) {
         fail(String.format(
               "(%s).traversePreorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               leftChild, list2.get(0), 0, leftChild, list2));
      }
      if (!list2.get(1).equals(grandChildA)) {
         fail(String.format(
               "(%s).traversePreorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               leftChild, list2.get(1), 1, grandChildA, list2));
      }
      if (!list2.get(2).equals(grandChildB)) {
         fail(String.format(
               "(%s).traversePreorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               leftChild, list2.get(2), 2, grandChildB, list2));
      }
      ...
      if (!list3.get(0).equals(grandChildA)) {
         fail(String.format(
               "(%s).traversePreorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               grandChildA, list3.get(0), 0, grandChildA, list3));
      }
   }
   ...
   public void testTraversePostorder() throws Exception {
      ...
      if (!list1.get(6).equals(parent)) {
         fail(String.format(
               "(%s).traversePostorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(6), 6, parent, list1));
      }
      if (!list1.get(2).equals(leftChild)) {
         fail(String.format(
               "(%s).traversePostorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(2), 2, leftChild, list1));
      }
      if (!list1.get(0).equals(grandChildA)) {
         fail(String.format(
               "(%s).traversePostorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(0), 0, grandChildA, list1));
      }
      if (!list1.get(1).equals(grandChildB)) {
         fail(String.format(
               "(%s).traversePostorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(1), 1, grandChildB, list1));
      }
      if (!list1.get(5).equals(rightChild)) {
         fail(String.format(
               "(%s).traversePostorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(5), 5, rightChild, list1));
      }
      if (!list1.get(4).equals(grandChildC)) {
         fail(String.format(
               "(%s).traversePostorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(4), 4, grandChildC, list1));
      }
      if (!list1.get(3).equals(greatGrandChild)) {
         fail(String.format(
               "(%s).traversePostorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(3), 3, greatGrandChild, list1));
      }
      final ArrayList<BinaryTreeNode<String>> list2 = new ArrayList<BinaryTreeNode<String>>();
      ...
      if (!list2.get(2).equals(leftChild)) {
         fail(String.format(
               "(%s).traversePostorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               leftChild, list2.get(2), 2, leftChild, list2));
      }
      if (!list2.get(0).equals(grandChildA)) {
         fail(String.format(
               "(%s).traversePostorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               leftChild, list2.get(0), 0, grandChildA, list2));
      }
      if (!list2.get(1).equals(grandChildB)) {
         fail(String.format(
               "(%s).traversePostorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               leftChild, list2.get(1), 1, grandChildB, list2));
      }
      final ArrayList<BinaryTreeNode<String>> list3 = new ArrayList<BinaryTreeNode<String>>();
   ...
   public void testTraverseInorder() throws Exception {
      ...
      if (!list1.get(3).equals(parent)) {
         fail(String.format(
               "(%s).traverseInorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(3), 3, parent, list1));
      }
      if (!list1.get(1).equals(leftChild)) {
         fail(String.format(
               "(%s).traverseInorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
      ...
      if (!list1.get(2).equals(grandChildB)) {
         fail(String.format(
               "(%s).traverseInorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(2), 2, grandChildB, list1));
      }
      if (!list1.get(6).equals(rightChild)) {
         fail(String.format(
               "(%s).traverseInorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(6), 6, rightChild, list1));
      }
      if (!list1.get(4).equals(grandChildC)) {
         fail(String.format(
               "(%s).traverseInorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
      ...
      if (!list1.get(5).equals(greatGrandChild)) {
         fail(String.format(
               "(%s).traverseInorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               parent, list1.get(5), 5, greatGrandChild, list1));
      }
      final ArrayList<BinaryTreeNode<String>> list2 = new ArrayList<BinaryTreeNode<String>>();
      ...
      if (!list2.get(1).equals(leftChild)) {
         fail(String.format(
               "(%s).traverseInorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
               leftChild, list2.get(1), 1, leftChild, list2));
      }
      if (!list2.get(0).equals(grandChildA)) {
         fail(String.format(
               "(%s).traverseInorder(...) visited %s on iteration %d, should have visited %s, visited nodes: %s",
